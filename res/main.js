...
var DrawCategory = /*#__PURE__*/function (DrawCategory) {
  DrawCategory["FREE"] = "category_free";
  DrawCategory["ANIMAL"] = "category_animal";
  DrawCategory["OBJECT"] = "category_object";
  DrawCategory["JOB"] = "category_job";
  DrawCategory["FOOD"] = "category_food";
  DrawCategory["LOL"] = "category_lol";
  DrawCategory["SPORT"] = "\uC2A4\uD3EC\uCE20";
  DrawCategory["POKEMON"] = "category_pokemon";
  return DrawCategory;
}(DrawCategory || {});
const _quizDB = {
  [Language.ko]: {
...
class GameRoom {
  ...
  constructor(roomNum, locationName) {
    ...
    //@ts-ignore
    this.locationInfo = Map.getLocationList(locationName)[0];
  }
  handleJoinPlayer(player) {
    if (this.gameStarted) {
  ...
  handleLeavePlayer(player, leaveMap = false) {
  ...
  initGame(drawerId = null) {
    ...
    const drawer = App.getPlayerByID(this.gamePlayInfo.drawerId);
    if (!drawer) {
      this.initGame();
      return;
    }
    ...
    //@ts-ignore
    drawer.tag.selectWidget.onMessage.Add(function (player, data) {
      ...
      if (type == "selectCategory") {
        ...
        if (category === "FREE") {
          ...
          {
            //@ts-ignore
            content: LocalizeContainer[player.language].prompt_desc,
            // 설명
            //@ts-ignore
            confirmVariant: 'primary',
            // 확인 버튼 색상 'primary' | 'alert'
            //@ts-ignore
          ...
          gameRoom.gamePlayInfo.currentQuizCategory = DrawCategory[category];
  ...
  handlePlayerToggleReady(player) {
    if (!this.validatePlayer(player)) return;
  ...
  processPlayerKickVote(player, targetId) {
    if (!this.validatePlayer(player)) return;
    let kickCount = 0;
    ...
    if (kickCount >= 3) {
      const targetPlayer = App.getPlayerByID(targetId);
      if (targetPlayer) {
  ...
  sendMessageToPlayerWidget(data) {
    for (let id of Object.keys(this.participatingPlayers)) {
      let player = App.getPlayerByID(id);
      if (!player) continue;
  ...
  handleDrawerSelectionTimeout() {
    const drawerPlayer = App.getPlayerByID(this.gamePlayInfo.drawerId);
...
class PlayerInfo {
  ...
  addExp(player, exp) {
    ...
    player.sendMessage(message, 0x00ffff);
  ...
  incrementGuessCorrectForMyDrawings() {
    this.guessCorrectForMyDrawings += 1;
  }
...
App.onLeavePlayer.Add(function (player) {
  ...
  if (_isMiniGame) {
    if (_drawerId === player.id) {
      App.runLater(() => {
        initGame(App.players[Math.floor(Math.random() * App.players.length)]);
      }, 0.1);
    }
  } else {
    if (player.tag.participatingRoomNum) {
...
function handleGameLobbyWidgetMessage(player, data) {
  switch (data.type) {
    case "join":
      {
        if (player.tag.participatingRoomNum) {
...
function initGame(player) {
  ...
  //@ts-ignore
  player.tag.selectWidget.onMessage.Add(function (player, data) {
    const type = data.type;
    if (type == "selectCategory") {
      let category = data.category;
      if (category === "FREE") {
        ...
        _currentCategory = DrawCategory[category];
...
const _GAMETIME = 70;
function startGame() {
  App.playSound("init.mp3", false, true);
  _gameTime = _GAMETIME;
  _start = true;
...
function showSubLabelTypeI(player, key, text1) {
  const isMobile = player.isMobile;
  // 10 : -2 //  58 : 49;
  ...
  if (_isMiniGame) {
    if (_start === true) {
      ...
      if (text === _currentQuiz || text === String(_currentQuiz).toLowerCase() || text === String(_currentQuiz).toUpperCase()) {
        _start = false;
        App.playSound("correct.mp3", false, true);
    ...
    const gameRoom = _gameRoomManager.getRoomByRoomNum(player.tag.participatingRoomNum);
    if (!gameRoom || !gameRoom.gameStarted || gameRoom.gamePlayInfo.state === STATE.INIT) return;
    ...
    if (text === gameRoom.gamePlayInfo.currentQuiz || text === String(gameRoom.gamePlayInfo.currentQuiz).toLowerCase() || text === String(gameRoom.gamePlayInfo.currentQuiz).toUpperCase()) {
      const answererName = player.name;
      const answer = gameRoom.gamePlayInfo.currentQuiz;
      gameRoom.actionToRoomPlayers(player => {
